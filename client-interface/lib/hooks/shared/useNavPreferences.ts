...
/** Stable sidebar ordering with explicit pin, reorder and optional usage sorting.
 * Saved manual preferences survive; legacy usage alone never moves items.
 */
export function useNavPreferences(role: UserRole) {
  ...
  const displayedOrder = useCallback(
    (): string[] => links.map((l: NavLink) => l.path),
    [links],
  );
  ...
  const togglePin = useCallback(
    (path: string) => {
      ...
      const nextPinned = current.includes(path)
  ...
  // Reset = back to the curated default (clear manual order, pins, and usage).
  const reset = useCallback(() => {
    persist({ order: [], pinned: [], manual: false, adaptive: false });
    clearNavUsage(role);
    setUsage({});
  }, [persist, role]);
  ...
  const recordUsage = useCallback(
    (path: string) => {
      if (typeof window === 'undefined' || !prefs?.adaptive) return;
