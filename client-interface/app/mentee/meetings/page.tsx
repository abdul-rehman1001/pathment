    ...
    <div className="space-y-6 max-w-5xl">
      <div className="mentee-page-heading">
        ...
        <p className="text-slate-600">
          Your guide, upcoming 1:1s, and the times they&apos;re free.
        </p>
      </div>
        ...
        <>
          <div className="grid gap-6 lg:grid-cols-3 items-start">
              ...
              {bookable.length === 0 ? (
                <div className="bg-card rounded-2xl border border-slate-200 p-8 text-center">
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    Once you&apos;re placed in a clan, your mentor shows up
                    here.
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex flex-wrap items-start gap-4">
                        <div className="w-14 h-14 rounded-full bg-brand-100 flex items-center justify-center shrink-0">
                          <span className="text-brand-700 text-lg font-semibold">
                            {initialsOf(b.mentor.name)}
                          </span>
                        </div>
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-4 flex flex-wrap items-center gap-2">
                            <Link
                              href={`/mentee/messages?participantId=${encodeURIComponent(b.mentor.id)}`}
                          ...
                          {b.slots.length === 0 ? (
                            <p className="mt-3 text-sm text-slate-400 rounded-xl bg-slate-50 border border-slate-100 px-3.5 py-2.5">
                              {b.mentor.name.split(" ")[0]} hasn&apos;t opened
                              any 1:1 times yet - message them to set some up.
                            </p>
                          ) : (
                            open && (
                              <div className="mt-4 rounded-xl border border-slate-200 p-3">
                                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 mb-2">
                                  Pick a time {b.mentor.name.split(" ")[0]} is
                                  free
                                </p>
                                <div className="flex flex-wrap gap-2 max-h-72 overflow-y-auto pr-1">
                                  {b.slots.map((s) => (
                                    <button
                                      ...
                                      onClick={() => {
                                        setBooking(s);
                                        setBookingMentor(b.mentor.name);
                                        setAgenda("");
                                      }}
                                    ...
                                    >
                                      ...
                                      {formatMeeting(s.startsAt, s.day, s.time)}
            ...
            <div className="space-y-3">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 px-1">
                Upcoming
              </p>
              {upcoming.length === 0 ? (
                <div className="bg-card rounded-3xl border border-border p-6 shadow-sm">
                  ...
                  <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                    Pick a time from your mentor&apos;s open slots and
                    it&apos;ll show up here.
                  </p>
                </div>
              ) : (
                upcoming.map((m) => (
                  <div
                    key={m.id}
                    className="bg-card rounded-2xl border border-slate-200 p-4"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-slate-900">
                          {formatMeeting(m.startsAt, m.day, m.time)}
                        </p>
                        <p className="text-xs text-slate-500">
                          {m.durationMins} min with {m.mentor?.firstName}
                        </p>
                      </div>
                      <button
          ...
          {past.length > 0 && (
            <div>
              ...
              <div className="bg-card rounded-2xl border border-slate-200 divide-y divide-slate-100">
                {past.map((m) => {
                  const cancelledByMentor =
                    m.status === "cancelled" &&
                    !!m.cancelledBy &&
                    m.cancelledBy === m.mentor?.id;
                  return (
                    <div key={m.id} className="px-5 py-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          <span
                            className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${m.status === "done" ? "bg-emerald-50" : "bg-slate-100"}`}
                          >
                            {m.status === "done" ? (
                              <Check className="w-4 h-4 text-emerald-600" />
                            ) : (
                              <CalendarX2 className="w-4 h-4 text-slate-400" />
                            )}
                          </span>
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-slate-400">
                              with {m.mentor?.firstName}
                            </p>
                          </div>
                        </div>
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs capitalize shrink-0 ${STATUS_CLASS[m.status] || "bg-slate-100 text-slate-500"}`}
                        >
                          {m.status}
                        </span>
                      </div>
                      {m.status === "cancelled" &&
                        (cancelledByMentor || m.cancellationReason) && (
                          <div className="mt-2 ml-10 rounded-lg bg-slate-50 border border-slate-100 px-2.5 py-1.5 text-xs text-slate-600">
                            {cancelledByMentor && (
                              <span className="font-medium text-slate-700">
                                Cancelled by your mentor.{" "}
                              </span>
                            )}
                            {m.cancellationReason ||
                              (cancelledByMentor ? "No reason given." : null)}
                          </div>
                        )}
                    </div>
                  );
      ...
      <Drawer
        ...
        subtitle={
          booking
            ? `${bookingMentor} · ${formatMeeting(booking.startsAt, booking.day, booking.time)} (${booking.durationMins} min)`
            : undefined
        }
        width="sm"
        footer={
      ...
      >
        ...
        <p className="mt-2 text-xs text-slate-400">
          Sharing an agenda helps your mentor prepare and makes the most of your
          time.
        </p>
      </Drawer>
      ...
      <Drawer
        ...
        subtitle={
          cancelFor
            ? `${formatMeeting(cancelFor.startsAt, cancelFor.day, cancelFor.time)} with ${cancelFor.mentor?.firstName ?? "your mentor"}`
            : undefined
        }
        width="sm"
        footer={
      ...
      >
        <p className="text-sm text-slate-600">
          This frees the slot so you (or another mentee) can book it again. Your
