...
export function RoadmapLinearStepCard({
  ...
  const getStatusBadge = () => {
    ...
    if (isCurrent) {
      return {
        text: "Current Stage",
        style: "bg-brand-100 text-brand-700 font-bold",
      };
    }
    return {
      text: `Step ${index + 1}`,
      style: "bg-slate-200/70 text-slate-600",
    };
  };
    ...
    <div className="relative flex items-start gap-3 sm:gap-5 group">
      ...
      <div className="flex flex-col items-center self-stretch shrink-0 pt-2.5">
        <button
          ...
          className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center border-2 z-10 transition-all focus:outline-hidden ${
            isDone
        ...
        {!isLast && (
          <div
            className={`w-0.5 flex-1 my-1.5 transition-colors ${
      ...
      <div
        className={`flex-1 min-w-0 rounded-2xl border transition-all duration-200 mb-3.5 ${
          isCurrent
          ...
          <div className="flex items-center gap-2 shrink-0">
            {step.assignedTask?.id ? (
              <button
                ...
                className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-xl text-[11px] sm:text-xs font-semibold flex items-center gap-1 transition-all shadow-2xs ${
                  isDone
        ...
        {isExpanded && (
          <div className="p-4 sm:p-5 pt-3 space-y-3.5 bg-card rounded-b-2xl">
            ...
            <div className="flex items-center gap-4 pt-1 text-xs text-slate-500 flex-wrap">
              ...
              {step.assignedTask?.dueDate && (
                <span
                  className={`inline-flex items-center gap-1 font-medium ${isOverdue ? "text-rose-600 font-bold" : "text-brand-600"}`}
                >
                  Due{" "}
                  {new Date(step.assignedTask.dueDate).toLocaleDateString(
                    "en-US",
                    {
                      month: "short",
                      day: "numeric",
