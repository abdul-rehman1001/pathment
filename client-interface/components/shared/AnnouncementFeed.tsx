    ...
    <div className="grid gap-5 xl:grid-cols-2 items-start">
      {announcements.map((a) => {
        ...
        return (
          <div
            key={a.id}
            className={`bg-card rounded-3xl border p-6 shadow-sm ${a.pinned ? 'border-brand-200' : 'border-slate-200'}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                  {a.author?.name}
                  {a.author?.role ? ` · ${a.author.role}` : ''} · to{' '}
                  {a.audienceLabel} · {timeAgo(a.at)}
                </p>
              </div>
              {manage && (
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() =>
                      act(a.id, () => announcementsApi.togglePin(a.id))
                    }
                    disabled={busy === a.id}
                    aria-label={
                      a.pinned ? 'Unpin announcement' : 'Pin announcement'
                    }
                    title={a.pinned ? 'Unpin' : 'Pin'}
                    className={`p-1.5 rounded-lg hover:bg-slate-100 ${a.pinned ? 'text-brand-600' : 'text-slate-400'}`}
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      act(a.id, () => announcementsApi.remove(a.id))
                    }
                    disabled={busy === a.id}
            ...
            <p className="text-sm text-foreground/85 mt-5 whitespace-pre-wrap leading-7">
              {a.body}
            </p>
            <div className="flex flex-wrap items-center gap-2 mt-5 border-t border-border pt-4">
              <button
                onClick={() =>
                  act(a.id, () => announcementsApi.react(a.id, 'acknowledged'))
                }
                disabled={busy === a.id}
              ...
              >
                ...
                Acknowledged{' '}
                {a.reactions.acknowledged > 0 && a.reactions.acknowledged}
              </button>
              <button
                onClick={() =>
                  act(a.id, () => announcementsApi.react(a.id, 'helpful'))
                }
                disabled={busy === a.id}
              ...
              >
                ...
                Helpful {a.reactions.helpful > 0 && a.reactions.helpful}
              </button>
            </div>
          </div>
        );
