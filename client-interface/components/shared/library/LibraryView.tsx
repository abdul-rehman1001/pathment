...
const CAT_META: Record<
  string,
  { icon: typeof BookOpen; cls: string; label: string }
> = {
  guidance: {
    icon: GraduationCap,
    cls: "bg-brand-50 text-brand-700 dark:bg-brand-500/15",
    label: "Guidance",
  },
  reading: {
    icon: BookOpen,
    cls: "bg-sky-50 text-sky-700 dark:bg-sky-500/15",
    label: "Reading",
  },
  template: {
    icon: FileText,
    cls: "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15",
    label: "Template",
  },
  policy: {
    icon: ShieldCheck,
    cls: "bg-amber-50 text-amber-700 dark:bg-amber-500/15",
    label: "Policy",
  },
};
  ...
  const cardProps = (d: LibraryDoc) => ({
    ...
    onRemove: async () => {
      if (
        ...
        act(d.id, () => libraryApi.remove(d.id), "Deleted");
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-2xl border border-slate-200 py-16 text-center text-slate-600">
          {error}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {pinned.length > 0 && (
            <section>
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400 mb-2 inline-flex items-center gap-1">
                <Pin className="w-3 h-3" /> Pinned
              </p>
              <div className="grid gap-4 md:grid-cols-2">
                {pinned.map((d) => (
                  <ResourceCard key={d.id} {...cardProps(d)} />
                ))}
              </div>
            </section>
          )}
...
function ResourceCard({
  ...
  return (
    <div
      className={`group relative bg-card rounded-3xl border p-6 flex flex-col transition-colors hover:shadow-sm ${d.pinned ? "border-brand-200" : "border-slate-200 hover:border-brand-300"}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div
          className={`w-12 h-12 rounded-2xl flex items-center justify-center ${meta.cls}`}
        >
        ...
        {canCurate && (
          <div className="flex items-center gap-0.5 opacity-100">
            <button
              ...
              className={`p-1.5 rounded-lg hover:bg-slate-100 ${d.pinned ? "text-brand-600" : "text-slate-400"}`}
            >
      ...
      <div className="flex items-center gap-2 mt-3">
        ...
        <span className="inline-flex items-center gap-1 text-[11px] text-slate-400">
          {d.hasContent ? (
            <>
              <FileIcon className="w-3 h-3" /> Article
            </>
          ) : (
            <>
      ...
      <button onClick={onOpen} className="mt-2 text-left">
        <h3 className="text-lg font-semibold text-foreground leading-snug group-hover:text-brand-700">
          {d.title}
        </h3>
        {d.summary && (
      ...
      <div className="mt-auto pt-5 border-t border-border flex items-center gap-2 text-xs text-slate-400">
        {d.author && <span className="truncate">{d.author}</span>}
        {d.readMins ? (
          <span className="inline-flex items-center gap-1">
            · <Clock className="w-3 h-3" /> {d.readMins} min
          </span>
        ) : null}
        <button
          onClick={onOpen}
...
function ReaderDrawer({
  ...
  useEffect(() => {
    libraryApi
      .get(docId)
      .then((r: any) => setDoc(r?.data?.document || null))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [docId]);

  const meta = doc
    ? CAT_META[doc.category] || CAT_META.guidance
    : CAT_META.guidance;
  ...
  return (
    <Drawer
      ...
      subtitle={doc ? meta.label : undefined}
      width="lg"
      footer={
        showFooter && doc ? (
          <div className="flex justify-between gap-2">
            {doc.url ? (
              <a
                href={toExternalHref(doc.url)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 text-slate-700 text-sm hover:bg-slate-50"
              >
      ...
      {loading ? (
        ...
        <div>
          <div className="flex items-center gap-3 text-xs text-slate-400 mb-4">
            ...
            {doc.readMins ? (
              <span className="inline-flex items-center gap-1">
                · <Clock className="w-3 h-3" /> {doc.readMins} min read
              </span>
            ) : null}
          </div>
          {doc.summary && (
            <p className="text-slate-600 mb-4 pb-4 border-b border-slate-100">
              {doc.summary}
            </p>
          )}
          {doc.content ? (
            <RichTextReader content={doc.content} />
          ) : (
            <p className="text-slate-500">
              This is a link-only resource. Use &ldquo;Open original&rdquo;
              below.
            </p>
          )}
        </div>
...
function EditorDrawer({
  ...
  const [readMins, setReadMins] = useState(
    doc?.readMins ? String(doc.readMins) : "",
  );
  ...
  useEffect(() => {
    ...
    libraryApi
      .get(doc.id)
      .then((r: any) => setContent(r?.data?.document?.content || ""))
      .catch(() => {})
  ...
  const save = async () => {
    ...
    if (!hasBody && !url.trim()) {
      toast.error("Add written content or a link");
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...
        url: url.trim(),
        readMins: readMins ? Number(readMins) : undefined,
      };
  ...
  return (
    <Drawer
      open
      onClose={onClose}
      title={doc ? "Edit resource" : "Add resource"}
      width="lg"
      footer={
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="e.g. How to give feedback that lands"
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">
            Summary <span className="text-slate-400">(shown on the card)</span>
          </label>
          <textarea
            rows={2}
            value={summary}
        ...
        <div>
          ...
          {loadingBody ? (
            <div className="py-10 flex justify-center border border-slate-200 rounded-lg">
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden">
              <RichTextEditor
                content={content}
                onChange={setContent}
                placeholder="Write the guidance, template, or notes here…"
                minHeight="220px"
              />
            </div>
          )}
          <p className="mt-1 text-xs text-slate-400">
            Write a full article, or leave this empty and just add a link below.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              External link <span className="text-slate-400">(optional)</span>
            </label>
            <input
              value={url}
